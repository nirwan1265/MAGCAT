...
Initially, we analyzed the leading genes from the sex-stratified GWAS to evaluate locus-level cross-sex reproducibility (Fig. A). SNP-to-gene annotation of principal GWAS signals, together with the retention of the top 50 genes per sex, demonstrated minimal overlap: merely 4 genes were common to both girls and males, whereas 46 were exclusive to each sex. Subsequent to SNP-to-gene aggregation, we employed MAGMA (Fig. B) to analyze SNPs inside a gene while considering the linkage disequilibrium structure. The genetic concordance was restricted, with 5 genes being shared and 45 being sex-specific. Consequently, whether genes are prioritized directly from GWAS findings or subsequent to MAGMA aggregation, the predominant gene lists are predominantly sex-specific, aligning with a polygenic framework wherein minor effect size estimates, linkage disequilibrium tagging of causal variations, and sampling variability can rearrange the top-ranked genes across strata.

Due to the susceptibility of gene-level rankings to polygenic characteristics, we investigated cross-sex repeatability in designated functional gene sets (pathways), where distributed enrichment is anticipated to exhibit greater stability than individual gene "top hit" lists. We utilized the correlation-aware omnibus calibration (Omni_MVN) and five component pathway statistics (ACAT, Fisher, adaptive TFisher, minP, and Stouffer) within the CATFISH framework to aggregate data employing a rank-based Top-K definition rather than a fixed p/FDR threshold. The limited and inconsistent route-level significance in these data results in strict criteria yielding highly varied and frequently sparse pathway sets (particularly in males), causing apparent overlap to primarily reflect statistical power rather than true concordance. Top-K contrasts pathway prioritizing by standardizing list length across genders and approaches. The primary pathway summaries utilize K = 20, as it is the minimum K at which a distinct six-way consensus junction is evident in both genders. At K = 15, the crossings are incomplete (maximum intersection size for males = 4; for females = 5), and no intersection is concurrently shared by all six approaches. Higher K values (50/100) expand the consensus core but generate denser intersection patterns that are less interpretable within a single main-text panel; hence, K-sensitivity is given independently.

The enrichment statistic significantly impacted the concordance of Top-20 pathways between females and males (Fig. C). ACAT and Fisher exhibited minimal sex overlap, with 1 and 2 shared pathways, respectively. Adaptive TFisher identified 5 shared pathways, whilst Stouffer and Omni_MVN demonstrated the highest concordance, each with 7 shared pathways. No shared pathways were identified in minP. Statistical mechanics forecasts this divergence: minP operates as a "winner-take-all" mechanism, predominantly influenced by the route's most significant gene. Consequently, even minor sex-specific alterations in the leading gene's identity or intensity can shift pathway rankings and eradicate Top-K overlap, despite the presence of weaker, dispersed signals. Stouffer (and frequently TFisher) exhibits heightened sensitivity to pathway-wide, distributed enrichment, which is more likely to be repeatable across sexes when multiple genes exert small effects.

We further investigated whether the exceptional cross-sex concordance of Omni_MVN might be attributed solely to its replication of one of the component tests. To analyze the omnibus signal, we compared the Top-20 Omni_MVN routes for each sex with the Top-20 pathways from each approach (Fig. D). ACAT (12/20) and minP (10/20) exhibited less overlap than Omni_MVN, which demonstrated the greatest overlap with Fisher in females (18/20 shared pathways). Omni_MVN had the highest alignment with Stouffer (18/20) and TFisher (17/20) in males, a moderate alignment with Fisher (10/20) and minP (11/20), and the lowest alignment with ACAT (8/20). In all comparisons, Omni_MVN preserved a significant subset of pathways absent from the component Top-20 list (and conversely), suggesting that the omnibus is not merely a reclassification of any singular statistic but rather an integrative prioritization that synthesizes multiple enrichment "modes" (e.g., diffuse signals captured by Stouffer/TFisher and aggregated evidence captured by Fisher).

The within-sex UpSet summary elucidate why an omnibus method exhibits greater stability than a singular component test (Fig. E–F). The Top-20 lists for females exhibit a consensus core of six methods comprising four pathways common to all component tests and Omni_MVN, alongside several moderate-sized partial intersections (3–4 pathways) and several singleton intersections (Fig. E). Males possess a larger six-method consensus core (six routes) and a fragmented overlap structure characterized by numerous method-specific or partially shared intersections (Fig. F). Collectively, these patterns indicate that (i) a true multi-method "core" signal exists within each sex, yet (ii) a significant portion of prioritized pathways is contingent upon the enrichment statistic employed—specifically the situation where an omnibus, correlation-aware synthesis can yield a more reproducible summary.

The data illustrates a coherent narrative: cross-sex concordance is restricted at the top-ranked gene level (Fig. A–B) but more evident at the pathway level (Fig. C), particularly for distributed enrichment-sensitive statistics (Stouffer and Omni_MVN). Omni_MVN functions as method integration rather than substitution (Fig. D), and the UpSet structure illustrates the utility of integration: pathway prioritization possesses a consensus core with numerous method-specific elements within each sex (Fig. E–F).
