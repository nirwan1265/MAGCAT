...
For a pathway \(S\) with \(G=|S|\) genes and nonnegative weights \(w_g\) (default \(w_g=1\)), define the **weighted Stouffer Z**:

\[
Z_{\mathrm{stouffer}}(S)=\frac{\sum_{g\in S} w_g\, Z_g}{\sqrt{\sum_{g\in S} w_g^2}}.
\]

The default **one-sided enrichment p-value** is:

\[
p_{\mathrm{stouffer}}(S)=\Pr\!\left(Z \ge Z_{\mathrm{stouffer}}(S)\right)=1-\Phi\!\left(Z_{\mathrm{stouffer}}(S)\right),
\]
...
**Permutation / MVN calibration (dependence-aware):** Because genes within pathways can be correlated (LD and genomic proximity), CATFISH can calibrate Stouffer p-values by recomputing \(Z_{\mathrm{stouffer}}(S)\) under a null generated by either (i) global resampling of gene Z’s, or (ii) MVN simulation using a pathway-specific gene–gene correlation matrix. For one-sided enrichment, the empirical p-value is computed as the fraction of null draws with \(Z_{\mathrm{stouffer}}^{\text{null}} \ge Z_{\mathrm{stouffer}}^{\text{obs}}\); for a two-sided option, the comparison uses \(|Z|\).
