...
We first examined the top genes from the sex-stratified GWAS to assess locus-level cross-sex reproducibility (Fig. A). SNP-to-gene annotation on lead GWAS signals and keeping the top 50 genes per sex revealed limited overlap. Only four genes were common to both female and males, whereas 46 were exclusive to each sex. We employed MAGMA (SNP-to-gene aggregation) (Fig. B) to analyze SNPs inside a gene while considering the linkage disequilibrium structure. Overlap at the gene level was sparse. Only five genes were common to both sexes and 45 were sex-specific. Consequently, whether genes are prioritized directly from GWAS results or following MAGMA aggregation, the gene lists are predominantly sex-specific, aligning with a polygenic framework where minor effect size estimates, LD tagging of causal variation, and sampling variability can reorder the top-ranked genes across strata.

Gene-level rankings are prone to polygenic signal dispersion, thus we tested cross-sex repeatability in terms of pathways, where dispersed enrichment is expected to be more stable than single-gene “top hit” lists. In the CATFISH framework, we used the correlation-aware omnibus calibration procedure (Omni_MVN) and five constituent pathway statistics (ACAT, Fisher, adaptive TFisher, minP, and Stouffer) to summarize the data using a rank-based Top-K definition instead of a p-value or FDR. Given these data's modest and heterogeneous pathway-level significance, stringent significance cutoffs can yield highly variable and often near-empty sets of significant pathways. So any apparent overlap reflects statistical power rather than biological concordance. The Top-K methodology standardizes pathway prioritization by requiring identical list lengths across sexes and analytical methodologies. The minimal K at which both sexes have a clearly defined six-way consensus intersection is K = 20, which we utilize in our principal pathway summary.

The enrichment statistic greatly altered female-male pathway ranking (Fig. C). Stouffer's approach and Omni_MVN had the maximum commanility (7 common routes each), whereas ACAT and Fisher's method had low overlap (1 and 2 shared pathways, respectively). MinP found no overlapping routes. The minP statistic is driven by the pathway's most important gene. Even when weaker, broadly distributed signals are shared between sexes, slight sex-specific variations in the lead gene's identity or effect size can significantly alter pathway rankings and minimize overlap. In contrast, Stouffer's technique (and often TFisher) is more sensitive to pathway-wide, distributed enrichment, making it more reproducible across sexes when many genes have minor effects.

We next examined whether the strong cross-sex concordance observed for Omni_MVN could be attributed to its effective duplication of one of the component tests. To decompose the omnibus signal, we compared the top 20 Omni_MVN-identified pathways within each sex to the top 20 pathways identified by each individual method (Fig. D). In females, Omni_MVN showed less overlap with ACAT (12/20 pathways in common) and minP (10/20) than with Fisher, with which it shared 18/20 pathways. In males, Omni_MVN was most similar to Stouffer (18/20 shared pathways) and TFisher (17/20), showed moderate concordance with Fisher (10/20) and minP (11/20), and was least aligned with ACAT (8/20). Omni_MVN is not a reparameterization of any single component test: despite strong overlap with individual methods, it consistently retains Omni-specific pathways and misses some method-specific pathways. This indicates that Omni_MVN integrates evidence across multiple enrichment statistics rather than duplicating one of them.

Finally, the within-sex UpSet plots explain why omnibus tests are more stable than single-component tests (Fig. E–F). In females, the lists show a six-method consensus core with 4 pathways jointly identified by all component tests and Omni_MVN, several moderately sized partial intersections (3–4 pathways), and many singleton intersections uniquely detected by individual methods (Fig. E). Males have a bigger six-method consensus core (6 routes) and a more fragmented overlap structure with many method-specific or partially shared intersections (Fig. F). The UpSet patterns indicate that each sex possesses a non-zero consensus core common to all six techniques (four pathways in females; six in men), whereas the majority of the Top-20 pathways reside within partial or method-specific crossings. Since no singular statistic encompasses all these junctions, an omnibus method that synthesizes across tests (and considers their correlation) offers a less test-dependent and hence more consistent pathway summary.

Thus, cross-sex concordance is limited at the top-ranked gene level (Fig. A–B) but observable at the pathway level (Fig. C), especially for distributed enrichment-sensitive statistics (Stouffer and Omni_MVN). Omni_MVN behaves like method integration, not replacement (Fig. D), and the UpSet structure shows why integration is helpful as pathway prioritization has a consensus core and many method-specific components inside each sex (Fig. E–F).
