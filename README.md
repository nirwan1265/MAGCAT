...
We first examined the top genes from the sex-stratified GWAS to assess locus-level cross-sex reproducibility (Fig. A). SNP-to-gene annotation on lead GWAS signals and keeping the top 50 genes per sex revealed limited overlap: only 4 genes were shared between females and males, while 46 were unique to each sex. After SNP-to-gene aggregation, we used MAGMA (Fig. B) to compare SNPs within a gene and account for LD structure. Genetic concordance was limited: 5 genes shared and 45 were sex-specific. Thus, whether genes are prioritized directly from GWAS hits or after MAGMA aggregation, the leading gene lists are mostly sex-specific, consistent with a polygenic setting where small effect size estimates, LD tagging of causal variation, and sampling variability can reorder top-ranked genes across strata.

Because gene-level ranks are vulnerable to polygenic features, we examined cross-sex reproducibility in specified functional gene sets (pathways), where distributed enrichment is expected to be more stable than single-gene “top hit” lists. We employed the correlation-aware omnibus calibration (Omni_MVN) and five component pathway statistics (ACAT, Fisher, adaptive TFisher, minP, and Stouffer) in the CATFISH framework to summarize data using a rank-based Top-K definition instead of a set p/FDR threshold Due to the small and inconsistent pathway-level significance in these data, stringent thresholds can return very variable and often near-empty pathway sets (especially in males), making apparent overlap mostly represent power rather than concordance. Top-K contrasts pathway prioritizing by enforcing identical list length across sexes and methodologies. The main pathway summaries use K = 20 because it is the smallest K at which a clear six-way consensus intersection is present in both sexes. At K = 15, intersections remain partial (male maximum intersection size = 4; female = 5) and no intersection is shared simultaneously by all six methods. K-sensitivity is presented separately since larger K values (50/100) increase the consensus core but produce denser intersection patterns that are harder to read in a single main-text panel.

The enrichment statistic greatly influenced female vs. male Top-20 pathway concordance (Fig. C). ACAT and Fisher had low sex overlap (1 and 2 shared pathways, respectively), adaptive TFisher revealed 5 shared pathways, while Stouffer and Omni_MVN showed the most concordance (7 shared pathways each). No common pathways were found in minP. Statistics mechanics predict this divergence: minP is basically “winner-take-all,” dominated by the route's most important gene, so even small sex-specific changes in the lead gene's identity or intensity can move pathway ranks and eliminate Top-K overlap even when weaker, scattered signal is shared. Stouffer (and often TFisher) is more sensitive to pathway-wide, distributed enrichment, which is more likely to be replicable across sexes when numerous genes contribute minor effects.

We then investigated whether Omni_MVN's remarkable cross-sex concordance could be explained by it duplicating a component test. To dissect the omnibus signal, we compared the Top-20 Omni_MVN routes within each sex to the Top-20 pathways from each technique (Fig. D). ACAT (12/20) and minP (10/20) overlapped less than Omni_MVN, which overlapped most with Fisher in females (18/20 shared pathways). Omni_MVN aligned most closely with Stouffer (18/20) and TFisher (17/20) in males, moderately with Fisher (10/20) and minP (11/20), and least with ACAT (8/20). In every comparison, Omni_MVN retained a non-trivial subset of pathways not in the component Top-20 list (and vice versa), indicating that the omnibus is not a relabeling of any single statistic but an integrative prioritization that draws from multiple enrichment “modes” (e.g., diffuse signal captured by Stouffer/TFisher and aggregated evidence captured by Fisher).

Finally, the within-sex UpSet summaries explain why an omnibus approach is more stable than a single component test (Fig. E–F). The Top-20 lists in females have a six-method consensus core of 4 pathways shared by all component tests and Omni_MVN, several moderate-sized partial intersections (3–4 pathways), and many singleton intersections (Fig. E). Males have a bigger six-method consensus core (6 routes) and a fragmented overlap structure with many method-specific or partially shared intersections (Fig. F). These patterns show that (i) each sex has a multi-method “core” signal, but (ii) a large fraction of prioritized pathways depends on the enrichment statistic used, in which case an omnibus, correlation-aware synthesis can provide a more reproducible summary.

The figure supports a consistent narrative: cross-sex concordance is limited at the top-ranked gene level (Fig. A–B) but more observable at the pathway level (Fig. C), especially for distributed enrichment-sensitive statistics (Stouffer and Omni_MVN). Omni_MVN behaves like method integration, not replacement (Fig. D), and the UpSet structure shows why integration is helpful: pathway prioritization has a consensus core and many method-specific components inside each sex (Fig. E–F).
